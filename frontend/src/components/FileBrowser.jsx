...
export const FileBrowser = () => {
    ...
    const generatePublicLink = async (filename) => {
        try {
            ...
            const resp = await fetch(
                `/api/link/generate?username=${cookies.user.username}&filename=${filename}`,
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${cookies.user.token}`
    ...
    const renderList = () => {
        const filteredFiles = userFiles.filter(file => file.toLowerCase().includes(searchQuery.toLowerCase()));
        const listItems = filteredFiles.map(file => {
            return <TableRow
                key={file}
            ...
            >
                <TableCell component="th" scope="row">
                    ...
                    <Box>
                        ...
                        <Button size="small"
                            onClick={() => generatePublicLink(file)}> Public Link </Button>
                        <Button size="small"
